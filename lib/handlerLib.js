...
const handlePostLogin = function(req, res) {
  ...
  client.query(query, (err, response) => {
    if (err) {
      ...
      selectQueryResult = response.filter(filterSelectResult);
      let registeredUsers = selectQueryResult[0].rows;
      let user = registeredUsers.find(u => u.user_name.toLowerCase() == req.body.userName.toLowerCase() && u.password == req.body.password);
      if (!user) {
      ...
      dbHandler.setSession(user, req, sessionid);
